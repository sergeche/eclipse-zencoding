...
public abstract class VariablePreferencePage extends PreferencePage implements
	...
	private static final String CONTEXT_TYPE_ID = ZenCodingContextType.CTX_VARIABLE;
...
//			composite.setLayoutData(new GridData());
		...
		protected IDialogSettings getDialogBoundsSettings() {
			...
			IDialogSettings settings= EclipseZenCodingPlugin.getDefault().getDialogSettings();
	...
	private void openWriteErrorDialog(IOException ex) {
		IStatus status= new Status(IStatus.ERROR, EclipseZenCodingPlugin.PLUGIN_ID, IStatus.OK, "Failed to write templates.", ex); //$NON-NLS-1$
		EclipseZenCodingPlugin.getDefault().getLog().log(status);
